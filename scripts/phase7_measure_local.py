...
def build_phase7_results_log(measurement: dict[str, Any], *, workflow_run_url: str | None = None) -> str:
    ...
    lines.append("- Profile: phase 7 local measurement")
    lines.append(f"- Workflow run URL: {workflow_run_url or 'local measurement run'}")
    lines.append("- Artifact location: local measurement output")
    lines.append("")
    ...
    lines.append("- Any evidence of a real bottleneck: no clear bottleneck from the measured L3 path")
    lines.append("- Any evidence of a deeper GPU opportunity: not from the current local evidence")
    lines.append(f"- Any reasons to keep attestation research-only: {attestation['details']}")
    lines.append("")
    lines.append("## Decision Summary")
    lines.append("- Is the current CPU path good enough?: yes, for the measured path")
    lines.append("- Is deeper GPU work justified now?: no, not from this evidence")
    lines.append("- Should anything move from research to product: no additional GPU work at this time")
    lines.append("")
